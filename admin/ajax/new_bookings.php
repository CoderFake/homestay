...
if(isset($_POST['get_bookings'])){
   ...
   WHERE bo.booking_status = 'pending' ORDER BY bo.booking_id ASC";
    ...
    while($data = mysqli_fetch_assoc($res)){
        ...
        $table_data .="
            <tr>
                ...
                <td>
                <button type='button' onclick=\"remove_room($data[booking_id])\" class='btn text-dark btn-outline-secondary shadow-none btn-sm' >
                XÁC NHẬN     
                </button>
...
if(isset($_POST['remove_room'])){
    ...
    // $res1 = select("SELECT * FROM `booking_order` WHERE `booking_id`=?",[$frm_data['booking_id']],'i');
    // if($res1){
    //     echo 1;
    // }
    $res3 = update("UPDATE `booking_order` SET `booking_status`=?  WHERE  `booking_id`=?",['confirm',$frm_data['booking_id']],'si');
